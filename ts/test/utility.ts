...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.STARTREK_TEST_LIVE ||
    'TRUE' === process.env.STARTREK_TEST_OVERRIDE
  ) {
  ...
  m.STARTREK_TEST_EXPLAIN = process.env.STARTREK_TEST_EXPLAIN || m.STARTREK_TEST_EXPLAIN
