...
//     (\`new StarTrekSDK(...)\` / \`StarTrekSDK.test(...)\` become
//     \`StarTrekSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"episode":{"test01":{"id":"test01"}},"spacecraft":{"test01":{"id":"test01"}},"species":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new StarTrekSDK(...)\` and any
// \`StarTrekSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+StarTrekSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/StarTrekSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
