...
describe('SpacecraftDirect', async () => {
  ...
  test('direct-list-spacecraft', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-spacecraft', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['STAR_TREK_TEST_SPACECRAFT_ENTID']
