...
boru-driven command-line client **and** interactive REPL for the StarTrek
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/star-trek-cli)
make build
...
./star-trek-cli list character
./star-trek-cli list episode

# 5. Override the API base URL for a single call
STAR_TREK_BASE=https://api.example.com ./star-trek-cli list character

# 6. No arguments -> interactive REPL
./star-trek-cli
star-trek> list character
star-trek> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/star-trek-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/star-trek-cli list character
   ```
...
Run with no arguments to open a REPL (prompt `star-trek>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/star-trek-cli`. |
...
(`./star-trek-cli <expr>`) and interactively (the REPL), and expressions compose the
