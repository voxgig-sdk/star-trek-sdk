// Command StarTrek-cli is a CLI + REPL for the StarTrek SDK,
...
const prompt = "star-trek"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character episode spacecraft species"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      STAR_TREK_APIKEY (api key), STAR_TREK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: STAR_TREK_APIKEY carries the API key and
	// STAR_TREK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("STAR_TREK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("STAR_TREK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewStarTrekSDK(opts)
