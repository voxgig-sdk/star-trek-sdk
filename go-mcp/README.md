...
[MCP](https://modelcontextprotocol.io) server exposing the StarTrek SDK as
two agent tools — `star-trek_list` and `star-trek_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/star-trek-mcp)
make build

# 2. Provide credentials via the environment
export STAR_TREK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user star-trek \
  -- /absolute/path/to/star-trek-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./star-trek-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// star-trek_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// star-trek_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/star-trek-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user star-trek \
     -- "$PWD"/dist/*/star-trek-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `star-trek_list` and `star-trek_load` tools now appear
   in new sessions. Ask the agent to *"list character using star-trek"*
   and it calls `star-trek_list` with `{"entity":"character"}`.
...
```sh
export STAR_TREK_APIKEY=sk_live_xxx            # API key
export STAR_TREK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `star-trek_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `star-trek_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `STAR_TREK_APIKEY` | API key sent with every request. |
| `STAR_TREK_BASE` | Optional override of the API base URL. |

### Entities

The 4 entities valid as the `entity` argument:

character | episode | spacecraft | species

### Smoke test via HTTP (raw JSON-RPC)

```sh
./star-trek-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"star-trek_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
