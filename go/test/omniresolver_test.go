...
import (
	...
	omni "github.com/voxgig-sdk/star-trek-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StarTrekError); is {
				out := map[string]any{
					"name":    "StarTrekError",
...
func omniCtx(arg any, client *sdk.StarTrekSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StarTrekSDK); is {
			client = live
