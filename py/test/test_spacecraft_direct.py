...
class TestSpacecraftDirect:

    def test_should_direct_list_spacecraft(self):
        setup = _spacecraft_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-spacecraft", "live" if setup["live"] else "unit")
        if _skip:
...
def _spacecraft_direct_setup(mockres):
