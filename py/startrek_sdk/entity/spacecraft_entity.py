...
from startrek_sdk.startrek_types import (
    Spacecraft,
    SpacecraftListMatch,
)
...
class SpacecraftEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SpacecraftEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Spacecraft]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Spacecraft().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
