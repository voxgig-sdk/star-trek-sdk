...
The Lua SDK for the StarTrek API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Character()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
| `stardateFrom` | `number` | Starting stardate |
| `stardateTo` | `number` | Ending stardate |
| `title` | `string` | Episode title |
| `uid` | `string` | Unique identifier |
| `usAirDate` | `string` | US air date |
| `yearFrom` | `number` | Starting year |
| `yearTo` | `number` | Ending year |
...
```
lua/
├── star-trek_sdk.lua    -- Main SDK module
...
The main module (`star-trek_sdk`) exports the SDK constructor
