...
local SpacecraftEntity = {}
SpacecraftEntity.__index = SpacecraftEntity


function SpacecraftEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SpacecraftEntity)
  self._name = "spacecraft"
  self._client = client
...
function SpacecraftEntity:get_name()
  return self._name
end


function SpacecraftEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SpacecraftEntity.new(self._client, opts)
end
...
function SpacecraftEntity:mark_deleted()
  self._deleted = true
end


function SpacecraftEntity:deleted()
  return true == self._deleted
end


function SpacecraftEntity:data_set(args)
  if args ~= nil then
...
function SpacecraftEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SpacecraftEntity:match_set(args)
  if args ~= nil then
...
function SpacecraftEntity:match_get()
...
function SpacecraftEntity:stream(action, args, callopts)
...
function SpacecraftEntity:_run_op(ctx, post_done)
  local utility = self._utility
