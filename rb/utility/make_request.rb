...
module StarTrekUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = StarTrekResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = StarTrekResponse.new(fetched)
    else
