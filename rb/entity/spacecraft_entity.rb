...
class SpacecraftEntity
  ...
  def make
    opts = @_entopts.dup
    SpacecraftEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = StarTrekHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = StarTrekHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #
  # @param reqmatch [SpacecraftListMatch, Hash, nil] match filter (any subset of
  #   Spacecraft fields); defaults to nil, treated as an empty match that lists all.
