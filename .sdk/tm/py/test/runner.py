...
class StarTrekTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            StarTrekTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = StarTrekTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = StarTrekTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = StarTrekTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = StarTrekTestRunner.getenv(key)
        ...
        explain = StarTrekTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if StarTrekTestRunner._test_control is not None:
            return StarTrekTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                StarTrekTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            StarTrekTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = StarTrekTestRunner.load_test_control()
...
def load_env_local():
    StarTrekTestRunner.load_env_local()


def env_override(m):
    return StarTrekTestRunner.env_override(m)


def entity_data(v):
    return StarTrekTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return StarTrekTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return StarTrekTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return StarTrekTestRunner.load_test_control()


def live_client_options():
    return StarTrekTestRunner.live_client_options()


def live_delay_ms():
    return StarTrekTestRunner.live_delay_ms()
