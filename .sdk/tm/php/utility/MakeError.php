...
// StarTrek SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class StarTrekMakeError
{
    public static function call(?StarTrekContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new StarTrekContext([], null);
        }
        $op = $ctx->op ?? new StarTrekOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new StarTrekResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof StarTrekError) ? $err->msg : (string)$err;
        $msg = "StarTrekSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new StarTrekError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof StarTrekError) {
