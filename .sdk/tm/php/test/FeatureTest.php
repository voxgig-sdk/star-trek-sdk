...
class FtCtrl extends StarTrekControl
{
...
class FtClient
{
    ...
    private StarTrekUtility $utility;

    public function __construct(StarTrekUtility $utility, array $options)
    {
    ...
    public function get_utility(): StarTrekUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StarTrekContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public StarTrekContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StarTrekContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StarTrekConfig::make_feature($name);
    ...
    private static function buildUrl(StarTrekSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StarTrekError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StarTrekResponse($fetched);
        }
    ...
    private function fail(StarTrekContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StarTrekContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
