...
class SpacecraftEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = StarTrekSDK::test($seed, null);
        $seen = iterator_to_array($base->Spacecraft(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = StarTrekConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = StarTrekSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Spacecraft(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STAR_TREK_TEST_SPACECRAFT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function spacecraft_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("STAR_TREK_TEST_SPACECRAFT_ENTID");
